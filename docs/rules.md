...
- **Запуск только как модуль**: приложение запускается командой `python -m app.main` из корня репозитория (working directory = корень проекта).
...
Запуск выполняется **из корня репозитория** (working directory = корень проекта), чтобы импорты вида `from app...` работали одинаково в терминале и в IDE.
